"""
Async SDPO Training with vLLM Inference Server.

Separates inference (vLLM on dedicated GPUs) from training (remaining GPUs).
This allows inference and training to run in parallel, maximizing GPU utilization.

Architecture:
- Inference GPUs: Run vLLM server for fast batched rollout generation
- Training GPUs: Run training loop with accelerate
- Completion Queue: Buffers completions between inference and training
...
def run_inference_loop(
    ...
    """
    Inference loop that runs in a separate thread.
    ...
    Args:
        ...
        dataloader: DataLoader yielding examples
        completion_queue: Queue for completions
        num_rollouts: Number of rollouts per example
        temperature: Sampling temperature
        max_tokens: Maximum tokens to generate
        num_epochs: Number of epochs to run
        weight_sync_event: Event signaling weight update available
        weight_sync_path_container: Container holding path to new weights
        stop_event: Event signaling stop
    """
...
def async_sdpo_train(
    ...
    Splits GPUs between inference (vLLM) and training (accelerate).
    Inference runs in a separate thread, feeding completions to the training loop.
    ...
    # Restrict training to training GPUs
    training_devices = ",".join(str(g) for g in async_config.training_gpu_ids)
    os.environ["CUDA_VISIBLE_DEVICES"] = training_devices
    logger.info(f"Training on GPUs: {training_devices}")

    # Initialize accelerator for training GPUs
    ...
    weight_sync_event = threading.Event()
    weight_sync_path_container: List[Optional[str]] = [None]
    stop_event = threading.Event()

    # Initialize vLLM client and start inference thread
    ...
    # Training loop - consume from completion queue
    try:
        while True:
            ...
            if batch is None:
                if completion_queue.is_shutdown() and completion_queue.empty():
                    logger.info("Queue empty and shutdown, training complete")
                    break
                continue

            # Process batch in groups (each group = rollouts for one example)
            # Since completions come from various examples, we process them together
            with accelerator.accumulate(model):
                # Get feedback for all completions
                feedbacks = []
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    log_dict = {
                        ...
                        "train/queue_size": completion_queue.size(),
                ...
                # Validation
                if accelerator.is_main_process and global_step % hparams.validation_interval == 0:
                    unwrapped_model = accelerator.unwrap_model(model)
